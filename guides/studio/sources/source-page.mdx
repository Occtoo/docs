...
<Info>
**Purge source** is disabled on a source with no entries ("This source has no entries to purge."), and **Delete source** is disabled on a source that has entries, cards or providers. So on any given source at most one of the two is usually available.
</Info>

## While a source is purging

A purge runs in the background. Until it finishes, the source is locked: **Import data** is disabled ("Source is being purged. Import is unavailable until it completes."), entries cannot be edited or deleted, and both **Purge source** and **Delete source** are disabled.
