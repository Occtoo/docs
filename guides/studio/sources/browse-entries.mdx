...
Click a column header to sort. Only properties with the **Indexing** capability are sortable — the rest render as plain headers, because the search index can only order on what it indexes. **Entry ID** and **Last modified** are always sortable. **Created** is not.
