...
<Steps>
  <Step title="Open Add Relation">
    ...
    Press **Add Relation**. Creating and editing relations needs the relation edit permission.

  </Step>
  <Step title="Name the relation">

    - **Name** — required, up to 50 characters.
    - **Identifier** — required, up to 50 characters, derived from the name. This is what destinations use.
    ...
    <Warning>
    The identifier must not match any property ID on either card — "The identifier must not match any property ID from the target or source card." It also cannot be changed after the relation is created.
    </Warning>
  ...
  <Step title="Pick the two sides">
    ...
    **Related From** is the card you are on. **Related to** is the card you are linking to; only **Active** cards are listed.
    ...
    On each side choose the **Property** that holds the matching value, and optionally a **Label** (up to 50 characters) naming that direction of the relation.
    ...
    <Info>
    The dialog states the rule for the property lists: "Relations are matched by index lookup, so only indexed properties can be selected. To use another property, enable its indexing capability first. The Id property is always available."
    </Info>

    <Warning>
    Two constraints are enforced when you save: the two sides cannot both be the `id` property ("Both source and target properties cannot be the ID property."), and a list property cannot be related to another list property ("A list property can not be related to a list property.").
    </Warning>
