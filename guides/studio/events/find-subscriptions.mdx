...
| **Pending verification** | A webhook whose endpoint has not answered the challenge yet. Nothing is delivered until it does — see [Verify an endpoint](/guides/studio/events/verify-an-endpoint) |
| **Paused** | Someone paused delivery. **Resume** starts again from the current event head |
