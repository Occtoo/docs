...
Types are joined with `or` and bracketed as a group once a condition is present; conditions are appended with `and`. A condition left without a value is dropped from the expression rather than sent empty.
