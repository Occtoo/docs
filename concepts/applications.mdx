...
Use **Try it** to request a token directly from this page. Enter the Application's client ID and client secret, then set the audience and optional scope for the API you want to access.
...
In **Try it**, set `audience` to the API version ID and leave `scope` empty.
...
In **Try it**, set `audience` to the Tenant ID and use `read:events`, `read:events:pull`, or `read:events:sse` as the scope.
