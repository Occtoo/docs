...
The tokens endpoint accepts a JSON payload in the body, consisting of the properties `clientId` and `clientSecret`. These properties correspond to the identifiers provided by the [application](/docs/occtoo-studio/guides/destination/how-to-register-an-application) used to access the destination.
