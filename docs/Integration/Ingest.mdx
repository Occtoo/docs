...
Ingesting data into Occtoo is facilitated by the Ingest API. This powerful API serves as the backbone of the data ingestion process, allowing both data and media to be seamlessly pushed into Occtoo. All [providers](/docs/Integration/Ingest/Providers) utilize this API via the [onboarding SDK](/docs/Integration#onboarding-sdk), acting as bridges between external systems and Occtoo's ecosystem.
...
![Data ingest](/images/integration-light.png)![Data ingest](/images/integration-dark.png)
</Accordion>
...
The tokens endpoint accepts a JSON payload in the body, consisting of the properties `id` and `secret`. These properties correspond to the identifiers provided by the [data provider](/docs/occtoo-studio/guides/sources/add-data-provider) used for the import.
...
Imports are initiated by sending a POST request to the URL ending with the identifier of the [data source](/docs/occtoo-studio/guides/sources/add-data-source) to which the data is intended for import.
...
Media ingestion can be performed using the ingest API, but the media file to be uploaded must be available at a public URL. The Occtoo [onboarding SDK](/docs/Integration#onboarding-sdk) supports uploading via streaming content directly. The import sequence must always start with a request for a token, as all imports require authentication. Once the token is obtained, the import endpoint can be called.
...
![Media upload via link sequence](/images/media-upload-via-links-light.png)![Media upload via link sequence](/images/media-upload-via-links-dark.png)
</Accordion>
...
![Get media upload status sequence](/images/media-upload-status-light.png)![Get media upload status sequence](/images/media-upload-status-dark.png)
</Accordion>
...
![Get media information by ID sequence](/images/media-information-by-id-light.png)![Get media information by ID sequence](/images/media-information-by-id-dark.png)
</Accordion>
...
![Get media information by unique identifier sequence](/images/media-information-by-unique-identifier-light.png)![Get media information by unique identifier sequence](/images/media-information-by-unique-identifier-dark.png)
</Accordion>
...
![Delete media sequence](/images/delete-media-light.png)![Delete media sequence](/images/delete-media-dark.png)
</Accordion>
