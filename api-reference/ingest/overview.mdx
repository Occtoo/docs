...
Data ingestion is performed using the ingest API. Two endpoints, _tokens_ and _import_, are necessary for completing a data import. The import sequence always needs to start with a request for a token, as all imports require authentication. Using the token, the import endpoint can then be called.

<Accordion title="Data import sequence visualzation">
  ![Data ingest](/images/api-reference/ingest/integration-light.png)

  ![Data ingest](/images/api-reference/ingest/integration-dark.png)
</Accordion>
...
<Tabs>
  <Tab title="Standard">
    <Tip>
      If an entry is to be added or updated, the **delete property** _can optianlly_ be omitted.
    </Tip>
...
<Accordion title="Media upload via link sequence visualzation">
  Upload media files using links via the [Upload via Links API](#upload-via-links).

  ![Media upload via link sequence](/images/api-reference/ingest/media-upload-via-links-light.png)

  ![Media upload via link sequence](/images/api-reference/ingest/media-upload-via-links-dark.png)
</Accordion>

<Accordion title="Get media upload status sequence visualzation">
  Get the upload status of a induvidual file using the Occtoo media file ID with the [Upload Status API](#upload-status).

  ![Get media upload status sequence](/images/api-reference/ingest/media-upload-status-light.png)

  ![Get media upload status sequence](/images/api-reference/ingest/media-upload-status-dark.png)
</Accordion>

<Accordion title="Get media information by ID sequence visualzation">
  Get detailed media information of a induvidual media file using the [Media File Information by File ID API](#media-file-information-by-file-id).

  ![Get media information by ID sequence](/images/api-reference/ingest/media-information-by-id-light.png)

  ![Get media information by ID sequence](/images/api-reference/ingest/media-information-by-id-dark.png)
</Accordion>

<Accordion title="Get media information by unique identifier sequence visualzation">
  Get detailed media information for one or multiple media files using the unique ID attribute with the [Media File Information by Unique Identifier API](#media-file-information-by-unique-identifier).

  ![Get media information by unique identifier sequence](/images/api-reference/ingest/media-information-by-unique-identifier-light.png)

  ![Get media information by unique identifier sequence](/images/api-reference/ingest/media-information-by-unique-identifier-dark.png)
</Accordion>

<Accordion title="Delete media sequence visualzation">
  Delete an uploaded media file with the [Delete Media File endpoint](#delete-media) using the assigned Occtoo Media File ID.

  ![Delete media sequence](/images/api-reference/ingest/delete-media-light.png)

  ![Delete media sequence](/images/api-reference/ingest/delete-media-dark.png)
</Accordion>
...
<Tip>
  If the media file that is to be upload contains [illegal characters](#rules-1) one can use the filename attribute to rename the file as this is the name that will be used for the file when uploaded to the Occtoo media service.
</Tip>
...
<Tabs>
  ...
  <Tab title="Without unique identifer">
    <Tip>
      It is possible to either set **uniqueidentifer** to _null_ or omit it entirely from the payload. Although it is our **strong recommendation** that a unique identifier is used to avoid reuploading the same binary multiple times.
    </Tip>
