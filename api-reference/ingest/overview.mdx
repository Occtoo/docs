...
The tokens endpoint accepts a JSON payload in the body, consisting of the properties `id` and `secret`. These properties correspond to the identifiers provided by the [data provider](/studio/sources/add-a-data-provider) used for the import.
...
Imports are initiated by sending a POST request to the URL ending with the identifier of the [data source](/studio/sources/add-a-data-source) to which the data is intended for import.
