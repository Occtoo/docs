...
<Update label="September 2026" description="Event subscriptions" tags={["Events", "Studio", "Sources", "Segments", "API"]}>
## Event subscriptions
...
## Redesigned data source page

The data source page is now the operational view of your source data: analytics widgets for entries, properties, and import activity, a language coverage report, and an entries browser with search, facets, and filters. Entries can be sorted by last modified and open in a detail sheet with **Data** and **Changelog** tabs.

## Tenant-level applications

A new **Settings → Applications** tab lets you create and manage applications for your whole tenant, with per-source and per-destination access and one-click access tokens. Data providers and destination applications are now tenant-level applications too, sharing one canonical scope model (`write:sources`, `events:pull`, …); existing applications were migrated automatically and legacy scope names continue to work.
...
<Update label="June 2026" description="Scheduled segment filters" tags={["Segments", "Ingest", "Studio"]}>
...
<Update label="February 2026" description="Destination publishing" tags={["Segments", "Destinations", "Studio", "Cards"]}>
